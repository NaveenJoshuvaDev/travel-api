...
- Here we do not have form request while we are using CLI for creating user.
- So that we don't have request variable.
- How to make a validation? using Terminal command or artisan command.
- Now we are going to hash the password last because we need to validate it first.
- you can create the hash in user observer and user model before saving.
- Below is the code with new changes and validation
...
```
- In this question  implementation we deliberately did not make automated tests 
- Because we automated test is for user-facing-feature use purpose, this is not for client user Interface
- Create automate test only for user facing front facing for the API.
- so we don't need that test.
- Here owner of the project will use terminal ,so we he will handle the errors.
- In this case its an internal command not an API.
- You can create an API for that as an Homework and put a PR request here.


### Admin users to create Travel Records
